...
export default function About({ setIsAboutOpen }: PropTypes) {
  return (
    <section className={styles.background}>
      <button className={styles.close_btn} onClick={() => setIsAboutOpen(false)} tabIndex={1}>
        <IoMdClose />
      </button>
        ...
        <table className={styles.table_container}>
          <tbody>
            {[...patch_notes].reverse().map(({ version, date, notes }, idx) => {
              return (
                <tr key={version} className={`${styles.row} ${idx == 0 ? styles.current : ''}`}>
